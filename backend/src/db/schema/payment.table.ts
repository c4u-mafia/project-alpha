...
export const payment = pgTable(
  'payment',
  {
    ...
    payeeId: text('payee_id').references(() => user.id, { onDelete: 'set null' }),
...
export const escrowHold = pgTable(
  'escrow_hold',
  {
    ...
    releaseScheduledAt: timestamp('release_scheduled_at', { withTimezone: true }).notNull(),
...
export const wallet = pgTable(
  ...
  (table) => [
    uniqueIndex('wallet_user_id_unique').on(table.userId),
  ],
);
